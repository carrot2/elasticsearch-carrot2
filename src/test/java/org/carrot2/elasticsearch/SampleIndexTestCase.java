...
public abstract class SampleIndexTestCase extends ESIntegTestCase {
  ...
  protected byte[] jsonResourceAs(String resourceName, XContentType toType) throws IOException {
    ...
    XContentBuilder builder =
        XContentFactory.contentBuilder(toType, baos).copyCurrentStructure(parser);
    builder.close();
  ...
  protected static Map<String, Object> checkHttpResponseContainsClusters(HttpResponse response)
      throws IOException {
  ...
  protected static Map<String, Object> checkHttpResponse(HttpResponse response) throws IOException {
    ...
    try (XContentParser parser =
        XContentHelper.createParser(
            ...
            XContentType.fromMediaTypeOrFormat(
                response.getFirstHeader(HttpHeaders.CONTENT_TYPE).getValue()))) {
      Map<String, Object> map = parser.map();
      Assertions.assertThat(map).describedAs(responseDescription).doesNotContainKey("error");
      return map;
    }
  }

  protected static void expectErrorResponseWithMessage(
      HttpResponse response, int expectedStatus, String messageSubstring) throws IOException {
    byte[] responseBytes = response.getEntity().getContent().readAllBytes();
    String responseString = new String(responseBytes, StandardCharsets.UTF_8);
    String responseDescription =
        "HTTP response status: "
